...
Each product has a price stored in its own dictionary along with the stock(quantity).
"""
# PRODUCT CATALOG
...
# This dictionary stores items that are added to the cart
cart = {}
...
It displays all available product with their prices and stock availability
"""
def display_products():
    print("\n  ======== Available Merchandise ========")

    # This loops through every product in the product catalog and print their details
...
It allows user to add items to cart. It validates stock availability
...
to add another item to cart without going back to main menu.
"""
def add_to_cart():
    while True:
        ...
        if items in products:
            try:
                ...
                # Validates if the quantity user entered is available
                if quantity <= 0:
                    print("\nInvalid Quantity!")
                    continue

                # If product quantity is sold out during session it will print out of stock
                ...
                # Checks if enough stock is available
                    ...
                    # This adds the selected item to cart or update the quantity
                    ...
                    # If user enters more stock than what's available
...
It allows user to remove items or quantity from cart
It updates the product dictionary immediately after confirmation
"""
def remove_from_cart():
    # This is confirmation if the cart is empty
    if not cart:
        print("\n   Cart is Empty!")
        return

    # This display items that are currently in the cart for users to choose what to remove
    view_cart()

    # Input asking for product name
    items = input("\nEnter Product Name to Remove: ").strip().title()

    # Validating if item exist in the cart
    ...
    while True:
        try:
            ...
            # If user is removing the all the available quantity in cart
            if quantity_to_remove == cart[items]:
                removed_quantity = cart[items]

                # This returns the removed stock back to inventory
                ...
                # This is a partial removal, only selected quantity will be removed
...
It allows user to view all items that was added to cart
It displays the price per item, the quantity and calculates total price
"""

def view_cart():
    # Display if there's nothing in cart to view
    if not cart:
...
It displays the cart summary, shows total calculation,
discount (12% if > 7000), tax at 10%, and it processes all payments
"""
def checkout():

    # Display if there's nothing in cart to check out
    if not cart:
        print()
    ...
    while True:
        try:
            ...
            if payments < total:
                ...
                # This clears cart after payment is successful
                cart.clear()
                break
        except ValueError:
            print("\nInvalid Input!")
...
It prints a formatted receipt that includes: header,
...
def generate_receipt(subtotal, discount, tax, total, payments, change):
    ...
    # Displays all the items purchased on receipt
    for items, quantity in cart.items():
        price = products[items]["Price"]
...
# This is the Main Function, and it's for choice selection and to have multiple sessions
"""
This is the MAIN PROGRAM FUNCTION, it's the main loop
It has the main menu system which allow users to 
